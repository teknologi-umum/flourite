...
export const Lua: LanguagePattern[] = [
  ...
  // local definition
  { pattern: /local\s(\w+)(\s*=)?/, type: "keyword.variable" },
  // function definition
  { pattern: /(local\s)?function\s*(\w*)?\(\)/, type: "keyword.function" },
  // for loop
  {
    pattern: /for\s+([a-zA-Z]+)\s*=\s*(\w+),\s*(\w+)\s+do/,
  ...
  // metatables
  {
    pattern: /((get|set)metatable|raw(get|set|equal))\(.*\)/,
    type: "keyword.other"
  },
  // metamethods
  {
    pattern: /__(index|newindex|call|sub|mul|div|mod|pow|unm|eq|le|lt)/,
    type: "keyword.other"
  },
  // method invocation
  ...
  {
    pattern: /(([a-zA-Z\d]+)::([a-zA-Z\d]+)|using|(.*)!\(.*\)|(\|\|))/,
    type: "not"
  }
];
