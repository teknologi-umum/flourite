...
export const Java: LanguagePattern[] = [
  ...
  {
    pattern: /(private|protected|public)\s*\w+\s*\w+(\s*=\s*\w)?/,
    type: "keyword"
  },
  // Method
  {
    pattern: /(private|protected|public)\s*(?:\w+\s+\w+|\w{2,})\(.+\)/,
    type: "keyword"
  },
  // String class
  ...
  // List/ArrayList
  {
    pattern: /(List<\w+>|ArrayList<\w*>\s*\(.*\))(\s+\w+|;)/,
  ...
  {
    pattern: /(^|\s)(char|long|int|float|double)\s+\w+\s*=?/,
    type: "constant.type"
  },
